...
import { useDriverChangeRequests } from "@/hooks/useProfileChangeRequests";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
import { UploadMedCardDialog } from "./UploadMedCardDialog";
import { UploadOtherDialog } from "./UploadOtherDialog";
...
export function PortalDocumentsTab() {
  ...
  const { data: requests = [] } = useDriverChangeRequests(driver?.id);
  ...
  const requestByDocId = useMemo(() => {
    const m = new Map<string, (typeof requests)[number]>();
    for (const r of requests) if (r.related_document_id) m.set(r.related_document_id, r);
    return m;
  }, [requests]);
  ...
  const handleAcknowledge = async () => {
    if (!viewing || !driver || !confirmed || signature.trim().length < 2) return;
    setSubmitting(true);
    try {
  ...
  const pick = (t: "license" | "medical" | "other") => {
    setPickerOpen(false);
    if (t === "license") setLicOpen(true);
    if (t === "medical") setMedOpen(true);
    if (t === "other") setOtherOpen(true);
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-end">
        <Button size="lg" className="gap-2" onClick={() => setPickerOpen(true)}>
              ...
              <Collapsible key={cat.id} defaultOpen={pending > 0}>
                ...
                <CollapsibleContent className="pt-2 space-y-2">
                  {items.map((doc) => {
                    ...
                    const req = requestByDocId.get(doc.id);
                    return (
                      <div
                        key={doc.id}
                        className={cn(
                      ...
                      >
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            {req && <RequestStatusPill status={req.status} note={req.review_note} />}
                          </div>
                          ...
                          {req?.status === "rejected" && req.review_note && (
                            <div className="mt-2 text-sm rounded-md bg-destructive/10 border border-destructive/30 text-destructive px-3 py-2">
                              Dispatch note: {req.review_note}
                            </div>
                          )}
                        </div>
      ...
      <Dialog open={pickerOpen} onOpenChange={setPickerOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>What kind of document?</DialogTitle>
            <DialogDescription>Pick a type to continue.</DialogDescription>
          </DialogHeader>
          <UploadTypePicker onPick={pick} />
        </DialogContent>
      </Dialog>

      <UploadLicenseDialog open={licOpen} onOpenChange={setLicOpen} />
      <UploadMedCardDialog open={medOpen} onOpenChange={setMedOpen} />
      <UploadOtherDialog open={otherOpen} onOpenChange={setOtherOpen} />
...
function RequestStatusPill({ status, note }: { status: string; note: string | null }) {
  if (status === "pending") {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border bg-amber-500/10 text-amber-500 border-amber-500/30">
        <Clock className="h-3 w-3" /> Pending dispatch review
      </span>
    );
  }
  if (status === "approved") {
    return (
      <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full border bg-emerald-500/10 text-emerald-500 border-emerald-500/30">
