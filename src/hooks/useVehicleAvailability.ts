...
export function useVehicleAvailability(
  ...
  const isOwnerScheduledToday = (vehicle: VehicleRow, allSchedules: DriverSchedule[]): boolean => {
    ...
    const ownerSchedule = allSchedules.find(
      s => s.driver_id === vehicle.assigned_driver_id && s.day_of_week === todayDayOfWeek
    );
    
    // If no schedule found, assume owner is OFF (vehicle available)
    if (!ownerSchedule) return false;
    
    // If is_off is true, owner is off today
    if (ownerSchedule.is_off) return false;
    
    // Owner is scheduled to work
    return true;
  };
  ...
  const hasValidTakeHomeOwner = (vehicle: VehicleRow): boolean => {
    if (vehicle.primary_category !== "above_all") return true;
  ...
  const isAvailableForDriver = (
    ...
    if (vehicle.classification === "take_home") {
      // If no owner assigned, flag as invalid (shouldn't be assignable)
      if (!vehicle.assigned_driver_id) return false;
      ...
      if (ownerWorking) {
        return isAdmin; // Admin can override
      }
